...
for (interaction_network in interaction_networks) {
  ...
  file_name <- paste0("nonrandom_probability_", interaction_network, ".csv")
...
#write.csv(A1,"probablity_interactionnetwork.csv")

b <- readRDS("metanetwork_structure_final.Rds")
b <- b %>%
  rename(Interaction_Network = network)

#A1 <- read.csv("probablity_interactionnetwork.csv", header = TRUE)
...
write.csv(A1,"nonrandom_total_data_fig3.csv")