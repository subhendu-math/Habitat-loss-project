...
for (interaction_network in interaction_networks) {
  ...
  for (k in 1:3) {
    ...
    for (jj in 1:length(r1)) {
      for (ii in 1:length(r1)) {
        ...
        full_filename <- paste0("random_dt_mutualism_", interaction_network, "_", network_type, "_er", er, "_ec", ec, "_cr", cr, "_cc", cc, ".csv")
  ...
  write.csv(A1, paste0("random_probability_", interaction_network, ".csv"))
}